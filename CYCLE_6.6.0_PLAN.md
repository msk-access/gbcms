...
> Every open finding after the 6.5.0 cut, whatever its priority. Sources: the
...
| ID | Ticket | Pri | Flags | Source |
|:--|:--|:-:|:--|:--|
| C1 | Local-alignment fallback reads stale semiglobal scores | H | [counts] | #92 |
| C2 | REF fragments at grouped rows (main vs per-transcript) | M | [counts] [decide] | 6.5.0 plan verification |
| C3 | Homopolymer decomposition arbitration redesign | M | [counts] | #111, #112 (1) |
| C4 | Reference windows near contig ends | M | [counts] | #92 |
| C5 | Long insertions exceed the pangenomic matrix cap | L | [counts] | 6.5.0 plan verification |
| C6 | Error-tolerant exact-length insertion matching | L | [counts] | #92 (enhancement) |
| C7 | Rescue for clip-borne ITD carriers | L | [counts] | #92 |
| C8 | One-base-REF delins without a shared anchor | L | [counts] | #116 review |
| C9 | Count a MAF deletion at Start 1 | L | [counts] | #116 review |
| R1 | Span-aware exon-edge BAQ rule | L | [counts] [decide] | #106 |
| R2 | RNA strandedness gating observability | M | [decide] | #114 |
| I1 | MAF allele base check | M | [decide] | #116 review |
| I2 | `End_Position` optional | L | | #116 review |
| I3 | VCF→MAF `Tumor_Seq_Allele1` | L | [decide] | #116 review |
| I4 | maf2vcf's second ALT from `Tumor_Seq_Allele1` | L | [decide] | #116 review |
| I5 | Nextflow `convert` module | L | | #116 |
| M1 | Merge rows whose flavors report different alleles | M | | #112 (2), 6.5.0 T7 |
| M2 | Merge inputs from different gbcms versions | M | | #116 review |
| M3 | Decomposed-allele hardening (observations, list length) | M | | #112 (3, 4) |
| O1 | UMI warning repeated by the rescue recount | L | | 6.5.0 plan verification |
| O2 | Run-start summary of enabled options | L | | 6.5.0 T7 |
| O3 | Rescue in fillouts without the MNP | L | | 6.5.0 T7 |
| H1 | Writers closed when a write fails | L | | #116 review |
| H2 | `is_indel` in preparation | L | | #116 review |
| P1 | Deep-bin fetch reduction (M5b) | L | | code-review plan |
| P2 | Bin cost-sort (PF-2) | L | | code-review plan |
| P3 | Document the bin-span soft floor (LO-3) | L | | code-review plan |
| S1 | Mean LLR per fragment (CR-5) | L | [decide] | code-review plan |
| S2 | `MIN_FOR_KS` floor (ME-9) | L | [decide] | code-review plan |
| D1 | CI version-consistency check | M | | 6.5.0 plan verification |
| D2 | Release workflow creates the GitHub Release | M | | 6.5.0 plan verification |
| D3 | mkdocs-material 2.0 | L | | docs build |
...
### C2 — REF fragments at grouped rows · M [counts] [decide]
...
### C5 — Long insertions exceed the pangenomic matrix cap · L [counts]
...
### C8 — One-base-REF delins without a shared anchor · L [counts]
...
### C9 — Count a MAF deletion at Start 1 · L [counts]
...
### I1 — MAF allele base check · M [decide]
...
### I3 — VCF→MAF `Tumor_Seq_Allele1` · L [decide]
...
### I4 — maf2vcf's second ALT from `Tumor_Seq_Allele1` · L [decide]
...
### M2 — Merge inputs from different gbcms versions · M
...
### O1 — UMI warning repeated by the rescue recount · L
...
### O2 — Run-start summary of enabled options · L
...
### O3 — Rescue in fillouts without the MNP · L
...
### S1 — Mean LLR per fragment (CR-5) · L [decide]
...
### S2 — `MIN_FOR_KS` floor (ME-9) · L [decide]
...
### D1 — CI version-consistency check · M
...
### D2 — Release workflow creates the GitHub Release · M
...
5. **Investigations and enhancements:** C5, C6, C7, C8, P1, then P2, O2, O3,
   I3, I4, I5 as decided.
6. **Infrastructure and docs:** D1, D2 (before the 6.6.0 cut), D3, P3.

The 6.6.0 cut is gated on steps 2–4 plus D1 and D2, with the same release-
