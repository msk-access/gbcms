...
- `BaseCounts` is frozen from Python (get-only fields, no mutators) — Python replaces a whole
  record rather than editing one (e.g. MNP rescue adopts the component SNV's `BaseCounts`)
